package com.taichuan.uilibrary.loadmoreview.listener;
...
public class ListScrollListener implements AbsListView.OnScrollListener {
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (!isShowFooter)
            Log.w(TAG, "不能加载更多 ");
        if (!loadMoreBaseView.isLoading() && scrollState == RecyclerView.SCROLL_STATE_IDLE && lastVisibleItem + 1 >= itemCount && isShowFooter) {
            if (loadMoreListener != null) {
                // 上拉加载更多
                loadMoreBaseView.setIsLoading(true);
